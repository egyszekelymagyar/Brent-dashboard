...
# =================================================================
# 1. KONFIGURÁCIÓ ÉS MEMÓRIA (TÖRVÉNY: MINDEN FUNKCIÓ MARAD)
# =================================================================
st.set_page_config(page_title="BRENT AI - PERMANENT TRADER", layout="wide", page_icon="🏦")

# Session State inicializálása - Ez biztosítja, hogy frissítésre megmaradjon az adat
...
# =================================================================
# 3. KERESKEDÉSI FUNKCIÓK (ÁLLANDÓ EGYENLEG KEZELÉSSEL)
# =================================================================
def manage_trade(action, side, price, risk=75):
    if action == "OPEN":
        # Csak akkor nyitunk, ha van elég pénz és nincs aktív ügylet
        ...
        if st.session_state.active_trade:
            ...
            st.session_state.wallet += profit # Frissítjük az állandó egyenleget
...
if df is not None:
    ...
    st.session_state.ai_broker = st.toggle("🤖 AI BRÓKER (AUTO MODE)", value=st.session_state.ai_broker)
    ...
    st.markdown(f"""<div class="mobile-grid">
        ...
        <div class="stat-card"><span class="stat-label">Vétel %</span><span class="stat-value">{buy_pct:.1f}%</span></div>
        <div class="stat-card"><span class="stat-label">Eladás %</span><span class="stat-value">{sell_pct:.1f}%</span></div>
    ...
    # 4. GRAFIKON (VIZUÁLIS ÚTVONAL Fix)
    fig = go.Figure()
    p_df = df.tail(80)
    ...
    if st.session_state.active_trade:
        ...
        if not segment.empty:
            ...
            fig.add_trace(go.Scatter(x=[segment.index[0]], y=[t['entry']], mode='markers', marker=dict(color='yellow', size=12, symbol='circle-dot'), name="BELÉPŐ"))
        
        fig.add_hline(y=t['entry'], line_dash="dash", line_color="yellow", annotation_text=f"BELÉPŐ: ${t['entry']:.2f}")
    ...
    if st.session_state.history:
        st.subheader("📜 Üzleti Napló")
        st.table(pd.DataFrame(st.session_state.history).tail(5))

    time.sleep(30)
    st.rerun()
else:
    st.warning("⏳ Adatfrissítés... Kérlek várj!")
    time.sleep(10)
    st.rerun()
