...
# =================================================================
# 2. DINAMIKUS ADAT ÉS ML MOTOR (MÚLT + JELEN)
# =================================================================
@st.cache_data(ttl=3600)
def load_historical_memory():
    """Fél év adatait elemzi a globális trendekhez."""
...
def get_dynamic_ai_prediction(hist_df, live_df):
    """Múltból tanuló, jövőre következtető dinamikus motor."""
...
def manage_trade(action, side, price):
    ...
    if action == "OPEN" and not st.session_state.active_trade:
        ...
        st.session_state.history.append({'Idő': datetime.now().strftime("%H:%M"), 'Profit': f"{profit:,.0f} Ft"})
...
if df is not None:
    ...
    # --- UI MEGJELENÍTÉS ---
    st.markdown(f'<div class="wallet-header"><h3 style="color:#f1c40f;margin:0;">VIRTUÁLIS EGYENLEG (TÉT: 75%)</h3><h1 style="color:white;margin:0;">{st.session_state.wallet:,.0f} Ft</h1></div>', unsafe_allow_html=True)
    
    # ROBOT PANEL (KAPCSOLÓVAL EGYÜTT)
    st.markdown('<div class="robot-header"><span class="robot-title">🤖 DINAMIKUS ROBOT BRÓKER</span>', unsafe_allow_html=True)
    st.session_state.ai_broker = st.toggle("ROBOT STATUS: AKTIVÁLÁS", value=st.session_state.ai_broker)
    ...
    st.markdown(f"""<div class="mobile-grid">
        ...
        <div class="stat-card"><span class="stat-label">Múlt elemzés</span><span class="stat-value">6 hónap</span></div>
        <div class="stat-card"><span class="stat-label">Vétel Esély</span><span class="stat-value">{'MAGAS' if buy_signal else 'ALACSONY'}</span></div>
        <div class="stat-card"><span class="stat-label">Eladás Esély</span><span class="stat-value">{'MAGAS' if sell_signal else 'ALACSONY'}</span></div>
    </div>""", unsafe_allow_html=True)

    # GRAFIKON (Sárga csillaggal és vastag vonallal)
    fig = go.Figure()
    plot_df = df.tail(60)
